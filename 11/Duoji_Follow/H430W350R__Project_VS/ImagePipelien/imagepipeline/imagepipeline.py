...
from utils import visualize, compute_mask_centers,visualize_centers,add_mask
...
def main_rs(frame_callback = None,save_output:bool  = False,save_video: bool = False, disable_vlm=False, video_source='camera', video_path=None):
    print("start!")
    # 参数验证
            ...
            while True:
                ...
                if not disable_vlm:
                    ...
                    if False:
                        ...
                        visualize(vis_frame, bboxes=bboxes if not initialized else None, mask=updated_mask,save_path=f"masked_{i}.png")
...
def main_rs_iter(save_output:bool  = False,save_video: bool = False, disable_vlm=False, video_source='camera', video_path=None):
    print("start!")
    # 参数验证
    if video_source == 'video' and video_path is None:
        raise ValueError("video_path is required when video_source is 'video'")
...
if __name__ == "__main__":
    ...
    for centers in main_rs_iter(save_video=False, disable_vlm=False,video_source='camera',video_path='/data/shiqi/ImagePipelien/imagepipeline/output.mp4'):
        print(centers)
