...
class RoboticsSystem:
        ...
        while True:
            if not self.in_post_run:
                ...
                if self.step > 0 and self.step % 10 == 0:
                    print(f"========================check_grasp_success{self.step}=============================")
                    # print()
                    # base64_image = encode_image_to_base64(self.third_color_image[..., ::-1])
                    # image_url = f"data:image/png;base64,{base64_image}"
                    head_image = self.robot.get_head_image(visulization=False)
                    with timer("check_grasp_success", self.log_file):
                        ...
                        ) and self.robot.right_hand.get_hand_position()[1] < 500:
    ...
    def mark_bbox_manual(self):
        head_image = self.robot.get_head_image()
        self.third_color_image = head_image
        # Save original image
        timestamp = time.strftime("%Y%m%d_%H%M%S")
    ...
    def run_planner(self):
        while True:
            ...
            while not self.talk_mode:
                self.step = 0
                if not self.get_current_instruction():
                    break
                ...
                while True:
                    ...
                    self.third_color_image = state['head_image'] #[..., [2, 1, 0]]
                    # cv2.imwrite("/data/yixiang/SRCB-DexVLA/temp/head_image.png", self.third_color_image )
                    self.right_arm_joint_feedback = state['arm_joint_positions'][1]
                    self.right_hand_joint = state['hand_joint_positions']
                    self.right_first_color_image = state['hand_image'] #[..., [2, 1, 0]]
                    # cv2.imwrite("/data/yixiang/SRCB-DexVLA/temp/hand_image.png", self.right_first_color_image )
                    # pdb.set_trace()
