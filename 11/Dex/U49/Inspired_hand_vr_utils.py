...
class Inspire_Hand_Interface():
    def __init__(self, read_topic, write_topic, speed_list):
        if not rospy.core.is_initialized():
            rospy.init_node('Hand_VR') # 初始化ROS节点
        self.get_hand_angle_topic = read_topic
        self.move_hand_topic = write_topic
        self.CMD_TYPE = ""
        self.HAND_ID = 1
        self.set_speed(speed_list)
        self.config = load_inference_config("/data/yixiang/SRCB-DexVLA/config.yaml")
        self.hand_config = config['robot']['hands']
        ...
        self.pub = rospy.Publisher('/hdas/feedback_gripper_right', JointState, queue_size=10)
        #self.pub = rospy.Publisher('/hand/angle_actual', Int32MultiArray, queue_size=10)
        # 动一个定时器，每 0.1 秒发布一次实际角度
        rospy.Timer(rospy.Duration(0.02), self.publish_actual_angles)
    ...
    def publish_actual_angles(self, event):
        """
        ...
        try:
            ...
            hand_joint_state.position = list(resp.curangle)
            # arr = Int32MultiArray(data=list(resp.curangle))
            self.pub.publish(hand_joint_state)
    ...
    def _on_right_gripper_cmd(self, msg):
        ...
        if pct != self.last_right_gripper_pct:  # 检查是否发生变化
            ...
            # 根据百分比（按键触发）决定是开合
            if pct > 50:    # 如果大于50，灵巧手张开
                self.move_hand(self.hand_config["right"]["default_standby"])
            else:   # 否则，灵巧手闭合
                self.move_hand(self.hand_config["right"]["default_grasp"])
...
if __name__ == "__main__":
    config = load_inference_config("/data/yixiang/SRCB-DexVLA/config.yaml")
    hand_config = config['robot']['hands']
    robot = Inspire_Hand_Interface(hand_config['right']['read_topic'], hand_config['right']['move_topic'], hand_config['right']['default_speed'])
